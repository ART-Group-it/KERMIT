...
var active_color = [0,0,137]
var inactive_color = [270, 255, 137]

f = chroma.scale(['black', 'grey', 'yellow', 'orange', 'red']);

function colAct(activation_level,rgb_index){
//LINEAR	return Math.round(active_color[rgb_index]*activation_level + inactive_color[rgb_index]*(1-activation_level));
//Quadratic
	
	return Math.round((active_color[rgb_index]-inactive_color[rgb_index])*activation_level*activation_level + inactive_color[rgb_index]);
}

function actToCSS(activation_level){
	return f(activation_level).css();
}
...
Node.prototype.set_width = function(ctx, vert_space, hor_space, term_font, nonterm_font) {
	...
	if (!this.has_children) {
        //IOOO: this.left_width = val_width / 2;
	...
	for (var child = this.first; (child != null) && (child.next != null); child = child.next) {
        ...
        //MARGINE TRA I SOTTOALBERI: Margine tra un sottoalbero e l'altro modificare il denominatore per mantenere le proporzioni DINDI
...
Node.prototype.draw = function(ctx, font_size, term_font, nonterm_font, color, term_lines) {
	...
	if (!this.has_children){
		activation_level = this.value.split(":")[1];
		tag = this.value.split(":")[0];
	}
	else{
		activation_level = this.value.split(":")[2];
		tag = this.value.split(":")[0] + ":"+ this.value.split(":")[1];
	}

	var local_font_size = Math.round(font_size*0.7*activation_level*activation_level)+Math.round(font_size*0.3)
	
	
	//ctx.fillStyle = "rgb(" +  colAct(activation_level,0) + "," + colAct(activation_level,1) + "," + colAct(activation_level,2) +")";
	ctx.fillStyle = actToCSS(activation_level,0);
	
	ctx.font = local_font_size + "px Verdana"
	//ctx.fillText(tag + ":" + activation_level, this.x, this.y);
	...
	if (this.draw_triangle) {
		...
		ctx.lineTo(this.x - this.left_width, this.y - local_font_size - padding_above_text);
		ctx.lineTo(this.x + this.right_width, this.y - local_font_size - padding_above_text);
	...
	ctx.beginPath();
	ctx.moveTo(this.parent.x, this.parent.y + padding_below_text);
	ctx.lineTo(this.x, this.y - local_font_size - padding_above_text);
	//ctx.strokeStyle="rgb(" +  colAct(activation_level,0) + "," + colAct(activation_level,1) + "," + colAct(activation_level,2) +")";
	ctx.strokeStyle= actToCSS(activation_level);
	ctx.stroke();
}
...
function draw_tree(str, font_size, term_font, nonterm_font, vert_space, hor_space, color, term_lines) {
	// Clean up the string
	str = str.replace(/^\s+/, "");
	console.log("str: draw",str);
	var open = 0;
	...
	// Set up the canvas.
    //width old -> var width = root.left_width + root.right_width + 2 * margin;
	var width = root.left_width + root.right_width + 2 * margin;

    //width height -> var height = root.max_y + font_size + 2 * margin;
