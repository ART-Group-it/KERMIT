...
class ActivationSubtreeLRP:
    ...
    def calculateDTFs(self, tree):
        ...
        :return: 
        '''
        for tt in self.kernel.subtrees(tree):
            self.kernel.dtf(tt)
    ...
    def saveActivation(self, tree, relevance_tree, tree_index_dict):
        ...
        return {'tree': tree, 'act_sub_trees': [[subtree, np.array([relevance_tree[tree_index_dict[subtree]]], dtype=np.float32)] for subtree in self.kernel.subtrees(tree)]}
