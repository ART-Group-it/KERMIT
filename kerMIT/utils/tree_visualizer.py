...
def activationInColor(i,min,max):
    ...
    final = (255,0,0)
    initial = (168,252,100)
    return rgb2hex(int(final[0]*act+initial[0]*(1-act)), int(final[1]*act+initial[1]*(1-act)), int(final[2]*act+initial[2]*(1-act)))
...
class TreeWidget(CanvasWidget):
    ...
    def __init__(self, canvas, t, make_node=TextWidget,
                 make_leaf=TextWidget, activation=0, minmax=(0,10), **attribs):
    ...
    def _make_collapsed_trees(self, canvas, t, key):
        ...
        # FMZ Expansion
        node_label = t.label().split(":")[0]
        if len(t.label().split(":")) == 2:
            node_attribs = self._nodeattribs.copy()
#            print(node_attribs)
            node_attribs["color"] = t.label().split(":")[1]
#            print(node_attribs)
        else:
            node_attribs = self._nodeattribs
        #node_label = t.label()


        node = make_node(canvas, node_label, **node_attribs)  # FMZ Expansion
        #if "color" in node_attribs:
        #    node["color"] = node_attribs["color"]
        if t.activation_level_initialized :
            node["color"] = activationInColor(t.activation()[self.get_activation()],self._minmax[0],self._minmax[1])

        #node = make_node(canvas, t.label().split(":")[0], **self._nodeattribs) # FMZ Expansion
...
#                print(node_attribs)
            ...
            ## FMZ
            if t.activation_level_initialized:
                local_line_color = activationInColor(t.activation()[self.get_activation()],self._minmax[0],self._minmax[1])
                local_line_width = 10 * (t.activation()[self.get_activation()] - self._minmax[0]) / (self._minmax[1] - self._minmax[0])



                # self._line_color = activation_color
                #for tseg in list(self._expanded_trees.values()):
                #    tseg['color'] = activation_color
                #    tseg['width'] = 10 * (t.activation()[self.get_activation()] - self._minmax[0]) / ( self._minmax[1] - self._minmax[0])


            treeseg = TreeSegmentWidget(canvas, node, subtrees,
                                        color=local_line_color,
                                        width=local_line_width)
            treeseg["color"] = local_line_color
